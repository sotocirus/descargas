...
echo
echo "####################### running SF_TEAM autostart scripts ####################### "
echo

##set temp output file
rm /tmp/SF.txt
touch /tmp/SF.txt
cat /proc/stb/info/boxtype >> /tmp/SF.txt
echo "" >> /tmp/SF.txt
cat /proc/version >> /tmp/SF.txt
echo "" >> /tmp/SF.txt
less /etc/version >> /tmp/SF.txt
echo "" >> /tmp/SF.txt
cat /proc/stb/info/chipset >> /tmp/SF.txt
echo "" >> /tmp/SF.txt
cat /proc/cpuinfo >> /tmp/SF.txt

##use transponder or ntd date and time
if [ -f /usr/lib/enigma2/python/Plugins/Extensions/SF_TEAM/scripts/dvbdate ]; then
	if [ -f /etc/enigma2/.transponderupdate ]; then
			echo "update time & date from multiplex"
			/usr/lib/enigma2/python/Plugins/Extensions/SF_TEAM/scripts/dvbdate --set --force --timeout 60 &
			if [ $? == 0 ]; then
			echo "update done ..."
			echo
			else
			echo "update failed ... try again"
			/usr/lib/enigma2/python/Plugins/Extensions/SF_TEAM/scripts/dvbdate --set --force --quiet --timeout 15 &
